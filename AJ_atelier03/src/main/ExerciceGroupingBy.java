...
public class ExerciceGroupingBy {
    ...
    private void groupBy1() {
        ...
        Map<Trader, List<Transaction>> traderListMap = transactions
                .stream()
    ...
    private void groupBy2() {
        ...
        Map<Trader, Long> traderLongMap = transactions
                .stream()
    ...
    private void groupBy3() {
        ...
        Map<TransactionsLevel, List<Transaction>> transactionsLevelListMap = transactions
                .stream()
                .collect(Collectors.groupingBy(trans -> {
                    if (trans.getValue() >= 1000 ) return TransactionsLevel.VERY_HI;
                    else if (trans.getValue() >= 800 && trans.getValue() < 1000) return TransactionsLevel.HI;
                    else if (trans.getValue() >= 600 && trans.getValue() < 800) return TransactionsLevel.ME;
