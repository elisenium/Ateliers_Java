...
public class EmployeeManagement {
    ...
    private static final Supplier<Stream<String>> supplier = () -> {
        ...
        try {
            return Files.lines(Paths.get("../AJ_atelier04/resources/streamingvf.csv"), Charset.defaultCharset());
        } catch (IOException e) {
    ...
    private static List<Integer> occurencesOfE() {
        ...
        BiFunction<Employee, Character, Integer> bF = (e, c) -> (int) e.getFirstname().chars().filter(ch -> ch == c).count();
        return supplier.get().map(Employee::new).map(e -> bF.apply(e, 'e'))
                .collect(Collectors.toList());
    }
    ...
    private static void printLongestName() {
        withLines( lines -> {
            // print le plus long nom de famille du fichier
