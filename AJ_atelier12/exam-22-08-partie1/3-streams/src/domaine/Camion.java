...
public class Camion  {
    ...
    public List<Trajet> trouverTrajetsAvecPlaceRestante(){
        return trajets
                .stream()
                .filter(e -> e.nbCaisses() < nbMaxCaisses)
    ...
    public LocalDate trouverDateTrajet(String villeDepart, String villeDArrivee){
        return trajets
                .stream()
                .filter(e ->
                        e.getDate().isAfter(LocalDate.now())
                                && e.getVilleDepart().equals(villeDepart)
                                && e.getVilleArrivee().equals(villeDArrivee))
                .sorted(Comparator.comparing(Trajet::getDate))
                .map(Trajet::getDate)
                .findFirst()
                .orElse(null);
    }
